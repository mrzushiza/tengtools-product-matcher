(function(){
  ...
  function specifications(value){
    ...
    function add(unit,number){var parsed=Number(number),converted=false;if(!isFinite(parsed))return;if(unit==='cm'){unit='mm';parsed*=10;converted=true;}if(unit==='inch'||unit==='inches'||unit==='in'||unit==='"'){unit='mm';parsed*=25.4;converted=true;}found.push({unit:unit,value:parsed,converted:converted});}
    ...
    raw.replace(/\b(\d+(?:\.\d+)?)\s*(mm|cm|tpi|nm|kg|inch|inches|in)\b/g,function(match,number,unit){add(unit,number);return match;});
  ...
  function badge(row){
    var label=row.status==='matched'?(row.matchType==='Manual match'?'Manually matched':'Matched'):row.status==='ignored'?'Ignored':('Review required'+(row.matchWarning?' · '+row.matchWarning:'')),kind=row.status==='matched'?'matched':row.status==='ignored'?'ignored':'review',symbol=row.status==='matched'?'confirmed':row.status==='ignored'?'ignore':'warning',indicators=[statusIndicator(kind,symbol,label)],duplicates=duplicateCount(row);
  ...
  function applyColumnWidths(){document.querySelectorAll('#tt-match thead th[data-column]').forEach(function(th){th.style.width=columnWidths[th.dataset.column]?columnWidths[th.dataset.column]+'px':'';});}
  ...
  function rowsFromGrid(grid){
    ...
    return grid.slice(start).map(function(line){var joined=line.map(text).filter(Boolean);if(!joined.length)return null;var description=di>=0?line[di]:(known?'':line.slice(1).join(' '));return {sourceLine:li>=0?line[li]:'',brand:bi>=0?line[bi]:'',itemId:ii>=0?line[ii]:(known?leadingItemCode(description):line[0]),description:description,quantity:qi>=0?line[qi]:'',unit:ui>=0?line[ui]:''};}).filter(Boolean);
  }
  ...
  function exportRow(r,component,index){
    ...
    return {'Source Line':r.sourceLine||r.row,'Import Row':r.row,'Input Brand':r.inputBrand,'Input Item ID':r.inputId,'Input Description':r.inputTitle,'Quantity':r.quantity,'Unit':r.unit,'Possible Duplicate':duplicates>1?'Yes':'No','Identical Line Count':duplicates,'Grouped Line':r.isGroup?'Yes':'No','Component No':component?index+1:'','Component Description':component?component.description:'','Component Qty per Set':component?component.quantity:'','Required Total':requiredTotal,'Match Status':item.status==='matched'?'Matched':item.status==='ignored'?'Ignored':'Needs review','Match Type':component?'Component match':r.matchType,'TengTools Item ID':item.tengSku,'TengTools Product Title':item.tengTitle,'TengTools Product Type':productClassLabel(item.productClass),'TengTools Product URL':item.tengHandle?location.origin+'/products/'+item.tengHandle:'','UNSPSC Code':item.unspsc,'UNSPSC Title':item.unspscTitle,'UNSPSC Version':component?'UNv260801':r.unspscVersion,'UNSPSC Confidence':component?'':r.unspscConfidence,'UNSPSC Review Status':component?'':r.unspscReviewStatus,'Match Reason':item.reason,'Review Decision':component?(item.status==='matched'?'Confirmed component':item.status==='ignored'?'Ignored component':'Needs component review'):r.decision};
  }
  ...
  document.addEventListener('mousemove',function(event){if(!resizeState)return;var delta=event.clientX-resizeState.startX,newWidth=Math.max(54,resizeState.startWidth+delta);resizeState.th.style.width=newWidth+'px';resizeState.table.style.width=Math.max(resizeState.startTableWidth+delta,resizeState.table.parentElement.clientWidth)+'px';columnWidths[resizeState.key]=Math.round(newWidth);});
  ...
  fetch(catalogUrl,{credentials:'same-origin'}).then(function(response){if(!response.ok)throw new Error('Product master unavailable');return response.json();}).then(async function(payload){products=catalogArray(payload).map(normalizeProduct).filter(function(p){return p.sku&&p.title;});applyUnspscFallbacks();indexProducts();catalogReady=true;setStatus(products.length.toLocaleString()+' TengTools products loaded with UNSPSC data.','success');var updated=await revalidateRestoredRows();if(updated)setStatus(products.length.toLocaleString()+' TengTools products loaded. Your saved review has been updated to the latest matcher.','success');}).catch(function(){setStatus('The TengTools product master could not be loaded. Please refresh the page.','error');});
